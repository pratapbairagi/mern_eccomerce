...
const Cart = ({ history }) => {
    ...
    // const { offerProducts } = useSelector(state => state.getProducts)
    ...
    useEffect(() => {
        if (cart) {
            let x = 0
            cart.forEach(item => {
                if(item.offer.avail === "no"){
                    x += +(item.price * item.quantity)
                }
                else if(item.offer.avail === "yes"){
                    x += +((item.price * item.quantity) - (item.price * item.quantity)*(item.offer.percentage/100))
                }
                        ...
                        return <div key={index} className="cartItem">
                            <div className="cartItemsContainer">
                                ...
                                <div className="col col-xl-1 col-2 cartItemTotal">
                                    <span className="cartItemTotalPrice" style={{textDecoration:`${item.offer.avail === "yes" ? "line-through" : "none"}`, color:`${item.offer.avail === "yes" ? "red" : "grey"}`}}>{(item.quantity * item.price)}</span>
                                    {item.offer.avail === "yes" &&
                                        <span className="cartItemTotalPrice">{(item.quantity * item.price) - ((item.quantity * item.price)*0.10)}</span>
                                    }
                                </div>
